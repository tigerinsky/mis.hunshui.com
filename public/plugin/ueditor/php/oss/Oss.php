...
class OSS {
    ...
    const IMG_BUCKET = 'hunshui';
    const INTERNAL_BUCKET = 'hunshui';
    const DOMAIN = 'http://img2.tigerinsky.com/';
    ...
    function upload($bucket, $file, $name) {
        try {
            ...
            error_log('oss exception: msg['.$e->getMessage().']'); 
            return false;
        }
    } 
    ...
    function upload_img($bucket, $file, $name) {
        ...
        curl_close($ch);
        if (false === $output) {
            error_log('oss: get img info error, url['.$url.']'); 
            return false;
        }
