...
class Publish_list_model extends MY_Model {
	private $dbr;
	private $dbw;
	...
	public function get_publish_info_by_plid($plid) {
		$this->dbw->select('plid, title, content, operator, ctime, utime, type, is_deleted');
		$this->dbw->where('plid', $plid);
	 ...
	 * @return bool 是否执行成功
	 */
	public function update_info($info, $plid){
		$where="plid={$plid}";
