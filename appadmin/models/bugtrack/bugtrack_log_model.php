 ...
 * bug管理model
 */
class Bugtrack_log_model extends CI_Model {
    ...
    public function get_data_by_parm($where, $limit){
        $query_data = "SELECT `id`, `bugtrack_id`, `old_handle_user`, `new_handle_user`, `type`, `ctime` FROM ci_bugtrack_log {$where} ORDER BY id ASC {$limit}";
    ...
    public function get_count_by_parm($where){
        $query_data = "SELECT count(id) as nums FROM ci_bugtrack_log {$where}";
        $result_data = $this->dbr->query($query_data);
    ...
    public function create_info($info){
        $insert_query=$this->db->insert_string('ci_bugtrack_log',$info);
