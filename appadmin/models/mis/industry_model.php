...
class Industry_model extends MY_Model {
    ...
    public function __construct() {
		...
		$this->table_name = "ci_app_industry";
		$this->table_name_short = "app_industry";
    }
    ...
    public function get_data_by_parm($where = "", $limit = ""){
        $query_data="SELECT `id`,`title` FROM ci_app_industry {$where} {$limit}";
    ...
    public function get_count_by_parm($where){
        $query_data="SELECT count(id) as nums FROM ci_app_industry {$where}";
        $result_data=$this->dbr->query($query_data);
    ...
    public function get_info_by_id($id){
        $query_data="SELECT `id`,`title` FROM ci_app_industry WHERE id=?";
    ...
    public function create_info($info){
        $insert_query=$this->db->insert_string('ci_app_industry',$info);
    ...
    public function update_info($info,$id){
        ...
        */
        $where="id={$id}";
        $update_rule=$this->db->update_string('ci_app_industry', $info, $where); 
    ...
    public function del_info($ids){
        ...
        $del_rule="DELETE FROM ci_app_industry WHERE id IN('{$id_str}') AND `status` =0";
