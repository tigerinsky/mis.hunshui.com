...
广告数据
*/
class Misad_model extends CI_Model {
    ...
    public function get_data_by_parm($where = "", $limit = ""){
        $query_data="SELECT `id`, `postion`, `rel_id`, `title`, `remark`, `go_url`, `img_url`, `status`, `flag`, `listorder`, `time_line`, `time_create` FROM ci_app_misad_info {$where} {$limit}";
    ...
    public function get_count_by_parm($where){
        $query_data="SELECT count(id) as nums FROM ci_app_misad_info {$where}";
        $result_data=$this->dbr->query($query_data);
    ...
    public function get_info_by_id($id){
        $query_data="SELECT `id`, `postion`, `rel_id`, `title`, `remark`, `go_url`, `img_url`, `status`, `flag`, `listorder`, `time_line`, `time_create` FROM ci_app_misad_info WHERE id=?";
    ...
    public function change_info_flag($ids_str){
        $update_change_query="UPDATE ci_app_misad_info SET `flag`=(`flag`+1)%2 WHERE id in('{$ids_str}')";
    ...
    public function change_info_status($ids_str){
        $update_change_query="UPDATE ci_app_misad_info SET `status`=(`status`+1)%2 WHERE id in('{$ids_str}')";
        if($this->db->query($update_change_query)){ 
            return true;
        }else{
            return false;
    ...
    public function change_info_order($listorders){
        $result=true;
        $edit_query="UPDATE ci_app_misad_info SET listorder=? WHERE id=?";
    ...
    public function create_info($info){
        $insert_query=$this->db->insert_string('ci_app_misad_info',$info);
    ...
    public function update_info($info,$id){
        ...
        */
        $where="id={$id}";
        $update_rule=$this->db->update_string('ci_app_misad_info', $info, $where); 
    ...
    public function del_info($ids){
        ...
        $del_rule="DELETE FROM ci_app_misad_info WHERE id IN('{$id_str}') AND `status` =0";
