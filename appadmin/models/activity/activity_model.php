...
class Activity_model extends CI_Model {
    ...
    public function get_data_by_parm($where,$limit){
        $query_data = "SELECT `id`, `name`, `type`, `position`, `img_url`, `jump_url`, `online_time`, `offline_time`, `is_deleted` FROM ci_mis_activity {$where} ORDER BY id DESC {$limit}";
    ...
    public function get_count_by_parm($where){
        $query_data = "SELECT count(id) as nums FROM ci_mis_activity {$where}";
        $result_data = $this->dbr->query($query_data);
    ...
    public function get_info_by_id($id){
        $query_data = "SELECT `id`, `name`, `type`, `position`, `img_url`, `jump_url`, `online_time`, `offline_time`, `is_deleted` FROM ci_mis_activity WHERE id=?";
    ...
    public function del_info($ids){
        ...
        $del_rule="UPDATE ci_mis_activity SET is_deleted = 0 WHERE id IN('{$id_str}')";
    ...
    public function create_info($info){
        $insert_query=$this->db->insert_string('ci_mis_activity', $info);
    ...
    public function update_info($info,$id){
        $where="id={$id}";
        $update_rule=$this->db->update_string('ci_mis_activity', $info, $where); 
