...
class Images {
    ...
    function __construct($params=array()) {
    	...
    	$this->thumb_enable = $params['thumb_enable'];
...
$this->set($this->site_setting['watermark_img'],$this->site_setting['watermark_pos'],$this->site_setting['watermark_minwidth'],$this->site_setting['watermark_minheight'],$this->site_setting['watermark_quality'],$this->site_setting['watermark_pct']);
    ...
    //返回最最后上传图片的高度和宽度
    function getpercent($srcwidth,$srcheight,$dstw,$dsth) {//后边两个参数为最大宽度和最大高度
    ...
    //添加水印
	function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = 'Faxhaidong',$w_font = 8, $w_color = '#ff0000') {
		...
		if(!empty($w_img) && file_exists($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font*2.5), 0, APPPATH.'third_party/database/font/elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
