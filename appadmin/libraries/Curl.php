...
/**
 * Description of CaseInsensitiveArray.php
 * 
 * @encoding UTF-8 
 * @date 2015-1l-25 12:38:38
 * @author 
 */
...
class Curl {
    ...
    private $_xml_pattern = '~^(?:text/|application/(?:atom\+|rss\+)?)xml~i';
...
//        $this->setDefaultJsonDecoder();
        ...
        if (is_array($data)) {
            if (self::is_array_multidim($data)) {
                if (isset($this->_headers['Content-Type']) &&
                        preg_match($this->_json_pattern, $this->_headers['Content-Type'])) {
    ...
    public function complete($callback) {
        $this->_complete_function = $callback;
    }
    ...
    public function exec($ch = null) {
        ...
        if (!$this->error) {
            $this->call($this->_success_function);
        } else {
            $this->call($this->_error_function);
        }

        $this->call($this->_complete_function);
...
//    }
    ...
    public function setHeader($key, $value) {
        ...
        $this->setOpt(CURLOPT_HTTPHEADER, array_map(array($this, 'map'), $headers, array_keys($headers)));
    }
    ...
    public function success($callback) {
        $this->_success_function = $callback;
    }
    ...
    private function parseResponse($response_headers, $raw_response) {
        ...
        if (isset($response_headers['Content-Type'])) {
            ...
            } elseif (preg_match($this->_xml_pattern, $response_headers['Content-Type'])) {
